...
public class UnsafeDemo3 {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            System.out.println("birkhoff-3-put0>>"+ Arrays.toString(demo0.getSalary()));
            //设置数组中第2个元素的值
            unsafe.putLong(demo0.getSalary(),(long)offset+scale*1,9);
            System.out.println("birkhoff-3-put1>>"+ Arrays.toString(demo0.getSalary()));
            unsafe.putLong(demo0.getSalary(),(long)offset+scale*2,9);
            System.out.println("birkhoff-3-put2>>"+ Arrays.toString(demo0.getSalary()));
            unsafe.putLong(demo0.getSalary(),(long)offset+scale*3,9);
            System.out.println("birkhoff-3-put3>>"+ Arrays.toString(demo0.getSalary()));
            unsafe.putLong(demo0.getSalary(),(long)offset+scale*4,9);
            System.out.println("birkhoff-3-put4>>"+ Arrays.toString(demo0.getSalary()));
            //设置第6个元素,不会自动扩容
            unsafe.putLong(demo0.getSalary(),(long)offset+scale*5,99);
            System.out.println("birkhoff-3-put5>>"+ Arrays.toString(demo0.getSalary()));

            //操作局部数组变量
            long[] array = new long[]{1,1,1,1};
            unsafe.putLong(array,(long)offset+scale*0,9);
            System.out.println("birkhoff-4-put0>>"+ Arrays.toString(array));
            unsafe.putLong(array,(long)offset+scale*1,9);
            System.out.println("birkhoff-4-put1>>"+ Arrays.toString(array));
            unsafe.putLong(array,(long)offset+scale*2,9);
            System.out.println("birkhoff-4-put2>>"+ Arrays.toString(array));

            //对数组的某个元素采用cas方法
            boolean result = unsafe.compareAndSwapLong(array,(long)offset+scale*3,9,10);
            System.out.println("birkhoff-5-cas1>>"+ Arrays.toString(array)+","+result);
            result = unsafe.compareAndSwapLong(array,(long)offset+scale*3,1,10);
            System.out.println("birkhoff-5-cas2>>"+ Arrays.toString(array)+","+result);
